...
class SaveBarcode {
  ...
  // }) async {
  //   await Gal.requestAccess(); // Request necessary permissions
  //   await ItemStorage.requestBackupPermissions();
  //   try {
  //     // Ensure the widget is fully rendered
  //     await Future.delayed(const Duration(milliseconds: 500));

  //     if (!cnx.mounted) return; // Check if widget is still in the tree

  //     if (repaintKey.currentContext == null) {
  //       if (cnx.mounted) {
  //         MyDialogs.showSnackBar(cnx, '❌ خطأ: لا يمكن العثور على العنصر!');
  //       }
  //       return;
  //     }
  ...
  //     if (byteData == null) {
  //       if (cnx.mounted) {
  //         MyDialogs.showSnackBar(cnx, '❌ خطأ: تعذر التقاط الصورة!');
  //       }
  //       return;
  //     }
  ...
  //     await Gal.putImage(tempPath, album: 'code_generatorGallery');

  //     if (cnx.mounted) {
  //       MyDialogs.showSnackBar(cnx, '✅ تم الحفظ في المعرض');
  //       Navigator.pop(cnx);
  //     }
  //   } catch (e) {
  //     if (cnx.mounted) {
  //       MyDialogs.showSnackBar(cnx, '❌ خطأ في حفظ الصورة: $e');
  //     }
  //   }
  // }
